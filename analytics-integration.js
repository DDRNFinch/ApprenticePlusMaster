...
function screenTransition(nextRaw){
 ...
 if(currentScreen!==next){
  ...
  currentScreen=next;screenEnteredAt=now;track('feature_open',{screen:next,category:screenCategory(next)});
 }
 if(TEST_SCREENS.has(next)&&!startedScreens.has(next)){startedScreens.set(next,now);track('assessment_started',{assessment_type:next});}
 if(RESULT_SCREENS.has(next)){const source=next.replace(/_result(s)?$/,'_test');const started=startedScreens.get(source)||startedScreens.get(next.replace('_result',''))||now;track('assessment_completed',{assessment_type:next,duration_seconds:Math.max(1,Math.round((now-started)/1000))});startedScreens.delete(source);}
}
...
function clickHandler(event){
 ...
 track('ui_action',info);
 const action=info.action,feature=info.feature;
 if(['download','export','print'].includes(action)||el.hasAttribute?.('download'))track('document_export',{screen:currentScreen,document_type:feature,action});
...
function init(){if(!A()){setTimeout(init,100);return}installScreenHook();installDownloadHooks();installLifecycle();track('analytics_integration_ready',{coverage:'full_ui_and_feature_flows'});}
