...
async function generateNVQEvidencePackPDF({course, assignment, profile, sections, branding, returnPackage=false}) {
  ...
  const walkthroughVideos=[];for(let vi=0;vi<(sections.discussion||[]).length;vi++){const version=sections.discussion[vi];for(const [code,rec] of Object.entries(version.recordings||{}))if(rec?.data)walkthroughVideos.push({code,rec,attempt:vi+1})}
  if(walkthroughVideos.length){
    const entries=[{name:pdfName,data:pdf}],used=new Set();
    walkthroughVideos.forEach(({code,rec,attempt},i)=>{const mime=String(rec.type||'video/webm'),ext=mime.includes('mp4')?'.mp4':mime.includes('ogg')?'.ogv':mime.startsWith('audio/')?'.webm':'.webm',base=safeZipName(`Attempt ${attempt} - ${code} Video Walkthrough`);let name=`${base}${ext}`;if(used.has(name.toLowerCase()))name=`${base}-${i+1}${ext}`;used.add(name.toLowerCase());entries.push({name:`Video Walkthrough Recordings/${name}`,data:dataUrlBytes(rec.data)})});
