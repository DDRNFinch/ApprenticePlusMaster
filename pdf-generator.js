...
async function generateEvidencePackPDF({course, assignment, profile, sections}) {
  ...
  // Cover page
  {
    ...
    const latestPractical=(sections.practical||[]).at(-1),latestSupport=(sections.supporting||[]).at(-1);y+=12;x.fillStyle=PALE;x.fillRect(M,y,W-2*M,112);label(x,'Latest practical result',M+22,y+34);value(x,latestPractical?practicalResultText(latestPractical):'Not submitted',M+300,y+34,20,true);label(x,'Latest witness / employer result',M+22,y+78);value(x,latestSupport&&latestSupport.tab!=='files'?resultText(latestSupport):(latestSupport?'Document evidence submitted':'Not submitted'),M+390,y+78,20,true);y+=142;
