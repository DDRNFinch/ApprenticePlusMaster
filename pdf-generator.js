// v1.38.0 complete evidence record export
'use strict';
...
async function generateEvidencePackPDF({course, assignment, profile, sections, branding, returnPackage=false, newEvidence={}, assignmentRpl=false, rplKsbCodes=[]}) {
  ...
  async function addCompleteEvidenceRecord({title,version,date,type,fields,signatureTitle='Signature',signatureData=''}){
    const blocks=(fields||[]).map(([labelText,content])=>[labelText,clean(content||'').trim()]).filter(([,content])=>content);
    let p=meta(newPage(`${title} - Complete Record`,version),version,date,type),x=p.x,y=sectionHeading(x,'Complete Submitted Details',p.y),pagePart=1;
    const continuation=()=>{pagePart++;p=meta(newPage(`${title} - Complete Record Continued`,version),version,date,type);x=p.x;y=sectionHeading(x,`Complete Submitted Details (continued ${pagePart})`,p.y)};
  ...
  for(let i=0;i<(sections.supporting||[]).length;i++){
    ...
    await addCompleteEvidenceRecord({title:`Supporting Evidence · SE${v}`,version:`Attempt ${v}`,date:d.date,type:d.type||'Supporting Evidence',fields:[['Evidence tab',d.tab],['Evidence type',d.type],['Name',d.personName],['Role',d.role],['Organisation',d.organisation],['Activity / description',d.activity],['KSBs evidenced',selectedKsbDetails(d)],['All marks awarded',scoredDetails(d)],['Assessment Summary',d.feedbackSummary],['Areas for Improvement',d.feedbackDevelopment],['Additional Comments',d.feedback],['Attached files',fileDetails(d)],['Photographs and captions',captionDetails(d)]],signatureTitle:`${d.type||'Supporting evidence'} signature`,signatureData:d.signature});await addFilePreviewPages(d,`Supporting Evidence · SE${v}`,`Attempt ${v}`,d.date,d.type||'Supporting Evidence');
  }
...
async function generateNVQEvidencePackPDF({course, assignment, profile, sections, branding, returnPackage=false, newEvidence={}, assignmentRpl=false, rplKsbCodes=[]}) {
  ...
  // Lossless complete records: every learner, witness and assessor field flows onto continuation pages instead of being clipped.
  for(let i=0;i<(sections.practical||[]).length;i++){const d=sections.practical[i],attempt=i+1;await addCompleteNvqRecord({title:'Assessor Observation',attempt,date:d.date,type:'Assessor Observation',fields:[['Assessor',d.tutor],['Activity observed',d.activity],['Task type',d.activityTaskType],['Specification ID',d.activitySpecId],['Learning Outcomes observed',selectedOutcomeDetails(d)],['All marks awarded',scoredOutcomeDetails(d)],['Assessor Observation / Additional Comments',d.feedback],['Assessment Summary',d.feedbackSummary],['Areas for Improvement',d.feedbackDevelopment],['Photographs and captions',photoOutcomeDetails(d)]],signatureTitle:'Assessor signature',signatureData:d.signature})}
  for(let i=0;i<(sections.photos||[]).length;i++){const d=sections.photos[i],attempt=i+1;await addCompleteNvqRecord({title:'Photographic Evidence',attempt,date:d.date,type:'Photographic Evidence',fields:[['Photographic Evidence Notes',d.activity],['Learning Outcomes evidenced',selectedOutcomeDetails(d)],['Photographs and captions',photoOutcomeDetails(d)]],signatureTitle:'Learner signature',signatureData:d.signature})}
  ...
  const supportingFiles=[];for(let vi=0;vi<(sections.supporting||[]).length;vi++){const version=sections.supporting[vi];for(const file of version.files||[])if(file?.data)supportingFiles.push({file,attempt:vi+1})}
  if(walkthroughVideos.length||supportingFiles.length){
    const entries=[{name:pdfName,data:pdf}],used=new Set();
    ...
    supportingFiles.forEach(({file,attempt},i)=>{const original=String(file.name||''),dot=original.lastIndexOf('.'),ext=dot>0?original.slice(dot):String(file.type||'').startsWith('image/')?'.jpg':'.bin',newMark=isNewEvidence('supporting',attempt)?'NEW EVIDENCE - ':'',base=safeZipName(`${newMark}${file.evidenceName||file.name||`Supporting Evidence ${i+1}`}`);let name=`${base}${ext}`;if(used.has(name.toLowerCase()))name=`${base}-${i+1}${ext}`;used.add(name.toLowerCase());entries.push({name:`Supporting Evidence Files/${name}`,data:dataUrlBytes(file.data)})});
